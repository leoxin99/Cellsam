"""
visualize_p75_binuclear.py

功能: 找到并可视化 P75 双核距离 (~246px) 的具体案例
"""
import numpy as np
import tifffile
import napari
...
print(f"找到 P75 (~246px) 双核案例:")
print(f"  文件: {closest['file']}")
print(f"  GT Cell ID: {closest['cell_id']}")
print(f"  核间距: {closest['distance']:.1f} px")
print(f"  核1: 面积={closest['nuc1'].area}px, 中心=({closest['nuc1'].centroid[1]:.0f}, {closest['nuc1'].centroid[0]:.0f})")
print(f"  核2: 面积={closest['nuc2'].area}px, 中心=({closest['nuc2'].centroid[1]:.0f}, {closest['nuc2'].centroid[0]:.0f})")
...
viewer = napari.Viewer(title=f"P75 Binuclear: dist={closest['distance']:.0f}px")
...
viewer.add_shapes(line, shape_type='line', edge_color='yellow', edge_width=3, 
                  name=f"Distance_{closest['distance']:.0f}px")
