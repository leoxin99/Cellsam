    ...
    def load_full_channels(self, sample_id):
        ...
        for tiff_dir in [self.data_dir, self.processed_dir]:
            ...
            if tiff_files:
                ...
                # Allen dataset channel layout per technical_details.md
                # Ch0=BF, Ch4=DAPI, Ch5=Actn2, Ch9=GT
                if img.ndim >= 2 and img.shape[0] >= 10:
                    return {
                        'bf': img[0],
                        'dapi': img[4] if img.shape[0] > 4 else None,
                        'actn2': img[5] if img.shape[0] > 5 else None,
