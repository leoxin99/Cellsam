#!/bin/bash -l
#SBATCH --job-name=best_l4
#SBATCH --partition=gpu-l4-24g
#SBATCH --gres=gpu:1
#SBATCH --time=72:00:00
#SBATCH --mem=48G
#SBATCH --cpus-per-task=8
#SBATCH --output=logs/best_l4_%j.log
#SBATCH --error=logs/best_l4_%j.err

# ============================================
# Best Config on L4: posw=10 + contour=off
# 2 runs: seed=42, seed=123
# + auto Oracle eval after each run
# Estimated: 2 × (~20h train + ~2min eval) = ~40h
# ============================================
...
echo "============================================"
echo "Best Config (L4): posw=10 + contour=off"
...
train_and_eval() {
  ...
  local eval_out="experiments/ablation_eval/best_config/seed${seed}_l4"
  mkdir -p "$eval_out"
