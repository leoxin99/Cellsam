...
### 2026-01-15 23:15 E15b 推理结果

#### 实验配置

- **实验ID**: E15b_inference_20260115_231151
- **模型**: `checkpoints/base_20260115_021255/best_model.pt` (E15b 多通道)
- **检测**: DAPI 核检测方案
- **输入**: BF + DAPI + Actn2 (3通道)

#### 结果

| 指标类型 | 指标 | 值 |
|----------|------|-----|
| **像素级** | Mean Pixel Dice | **0.7203** |
| 实例级 | Mean Instance Dice | 0.0000 |
| 检测 | Detection F1 | 0.0000 |
| 检测 | TP / FP / FN | 0 / 26 / 124 |

#### 分析

**像素级 Dice 0.7203 说明模型确实在工作**，但实例级匹配失败：

1. **IoU 匹配阈值问题**: 当前设置 IoU >= 0.5 才算匹配
2. **预测覆盖问题**: 预测区域与 GT 区域位置偏移导致 IoU < 0.5
3. **检测失配**: DAPI 检测的框位置与 GT 细胞位置不完全对应

**待改进**:
- 降低 IoU 匹配阈值至 0.3 或 0.1
- 改进 DAPI 检测的框扩展参数
- 对比 E12 模型在相同数据上的表现

---

### train_expanded.py 历史

根据 git log:
- **创建时间**: 项目早期 (commit 23570c5 "Add CellSAM training scripts")
- **用途**: 完整训练脚本，包含早停、AMP 等功能
- **当前状态**: 未使用，已有早停功能移植到 src/train.py

---

### 2026-01-16 01:30 E12 vs E15b 对比结果
...
- 样本数: 5
- E12: BF × 3 输入
- E15b: BF + DAPI + Actn2 输入

#### 结果

| 指标 | E12 | E15b | 差异 |
|------|-----|------|------|
| Mean Pixel Dice | **0.7718** | 0.7454 | **-0.0263** |
| Mean Instance Dice | 0.0000 | 0.0000 | - |
| Detection F1 | 0.0000 | 0.0000 | - |

#### 关键发现

1. **E12 更好**: E12 Pixel Dice 比 E15b 高 2.6%
2. **多通道无效**: DAPI + Actn2 没有提升分割效果
3. **E15b 分割全屏单色**: 用户观察到模型输出为全屏同一颜色
...
原因:
- 模型未能学习到 DAPI/Actn2 通道信息
- SAM Image Encoder 预训练于 RGB 自然图像
- 直接替换通道没有语义对应
...
| 优先级 | 任务 | 说明 |
|--------|------|------|
| **P0** | 边界平滑处理 | E12 分割边界有锯齿 |
| **P0** | 全数据训练 | 478 样本 |
| P1 | 降低 IoU 阈值 | 解决实例匹配问题 |
| P1 | LR 调参实验 | 1e-5, 5e-5, 1e-4 |
| ~~P1~~ | ~~通道注意力~~ | ~~已废弃~~ |
| ~~P2~~ | ~~通道消融~~ | ~~已废弃~~ |
