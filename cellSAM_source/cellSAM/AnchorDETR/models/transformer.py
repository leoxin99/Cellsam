...
class Transformer(nn.Module):
    def __init__(
        ...
        encoder_layer = TransformerEncoderLayerSpatial(
            d_model, dim_feedforward, dropout, activation, nhead, attention_type
        )
        encoder_layer_level = TransformerEncoderLayerLevel(
        ...
        self.num_encoder_layers_spatial = (
            num_encoder_layers - self.num_encoder_layers_level
        )
    ...
    def forward(
        ...
        tgt = (
            ...
            .repeat(bs, 1, self.num_position, 1)
        ...
        if self.attention_type == "RCDA":
            ...
            pos_2d = torch.cat(
                [
                    pos_row.unsqueeze(1).repeat(1, h, 1).unsqueeze(-1),
                    pos_col.unsqueeze(2).repeat(1, 1, w).unsqueeze(-1),
                ],
                dim=-1,
            )
        ...
        for idx in range(len(self.encoder_layers)):
            ...
            if idx < self.num_encoder_layers_level:
                outputs = self.encoder_layers_level[idx](
                    outputs,
                    level_emb=self.level_embed.weight.unsqueeze(1)
                    .unsqueeze(0)
                    .repeat(bs, 1, 1, 1)
                    .reshape(bs * l, 1, c),
                )
...
class TransformerEncoderLayerSpatial(nn.Module):
    ...
    def forward(
        self, src, padding_mask=None, posemb_row=None, posemb_col=None, posemb_2d=None
    ):
        # self attention
        bz, c, h, w = src.shape
        ...
        if self.attention_type == "RCDA":
            ...
            src2 = (
                self.self_attn(
                    (src + posemb_row).reshape(bz, h * w, c),
                    (src + posemb_col).reshape(bz, h * w, c),
                    src + posemb_row,
                    src + posemb_col,
                    src,
            ...
            src2 = (
                self.self_attn(
                    ...
                    key_padding_mask=padding_mask.reshape(bz, h * w),
                )[0]
                .transpose(0, 1)
                .reshape(bz, h, w, c)
            )
...
class TransformerEncoderLayerLevel(nn.Module):
    def __init__(
        self, d_model=256, d_ffn=1024, dropout=0.0, activation="relu", n_heads=8
    ):
        super().__init__()
    ...
    def forward(self, src, level_emb=0):
        ...
        src2 = self.self_attn_level(
            src.reshape(bz, h * w, c) + level_emb,
            src.reshape(bz, h * w, c) + level_emb,
            src.reshape(bz, h * w, c),
        )[0].reshape(bz, h, w, c)
    ...
    def forward(
        ...
        if self.attention_type == "RCDA":
            ...
            tgt2 = self.cross_attn(
                (tgt + query_pos_x).repeat(l, 1, 1),
                (tgt + query_pos_y).repeat(l, 1, 1),
                k_row,
                k_col,
                srcs,
                key_padding_mask=src_padding_masks,
            )[0].transpose(0, 1)
        else:
            tgt2 = self.cross_attn(
                (tgt + query_pos).repeat(l, 1, 1).transpose(0, 1),
                (srcs + posemb_2d).reshape(bz * l, h * w, c).transpose(0, 1),
                srcs.reshape(bz * l, h * w, c).transpose(0, 1),
                key_padding_mask=src_padding_masks.reshape(bz * l, h * w),
            )[0].transpose(0, 1)

        if l > 1:
            tgt2 = self.level_fc(
                tgt2.reshape(bz, l, tgt_len, c)
                .permute(0, 2, 3, 1)
                .reshape(bz, tgt_len, c * l)
            )
